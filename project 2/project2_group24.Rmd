...
reduced.model<-lm(Outstate~Private+Room.Board+Terminal+perc.alumni+Expend,college.train)
summary(reduced.model)
```

The reduced model is
$$ \begin{aligned}
\textbf{Outstate} &=  - 4047.0191 + 2847.0626 \textbf{Private}+1.2261 \textbf{Room.Board}\\
&+ 43.0779 \textbf{Terminal}+ 75.1039  \textbf{perc.alumni} +  0.1983  \textbf{Expend}, 
\end{aligned}$$
...
```
The test MSE is
$$MSE = \frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{f(x_i)})^2 =  4010675$$
...
```{r, echo=TRUE, eval=TRUE,fig2,fig.cap="MSE for different values of log(lambda). \\label{fig:fig2}"}
set.seed(2)
...
From Figure 2 we can observe that the optimal tuning parameter is $\lambda = 120.4282$ as this is the parameter that minimizes the MSE for the training set. 

The test MSE is now $3744943$, which is lower than what we found for the reduced model using forward selection in d).
...
library(ggplot2)
?ggplot
ggplot(college.train, mapping = aes(x=Room.Board,y=Outstate) + geom_point() )
