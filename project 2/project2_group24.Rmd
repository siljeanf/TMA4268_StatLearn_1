...
We see that all three linkage with the euclidian distance perform perfectly. On the other hand, the correlation-based distance is not optimal. With complete and average linkage it puts too many observations in the second group. The single linkage puts all observations in one group, except one.
Therefore when we know the true state of the tissue, the euclidian distance performs best, with any linkage.
...
cumsum(pve)[5]
```
21 percent of variance explained by first 5 PC (?)
