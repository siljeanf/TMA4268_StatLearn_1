...
```
The training MSE for the two methods are (Read about this!)
...
TRUE, TRUE, TRUE, FALSE (not sure about last)
...
So by looking at these rates the support vector classifier with a linear boundary seems to make the best classification rule for the prescence of diabetes for our data. This is because the number of missclassifications is remarkable lower for this classifier.
...
Both logistic regression and support vector machines solve classification problems. In logistic regression all observations contribute weighted to the decision boundary, while for SVMs, only the support vectors (the points closest to the decision boundary) contribute to the margin. A consequence of this is that LR is more sensitive to outliers than SVM. On the other hand, LR can be easily updated as new data comes in.
...
FALSE, FALSE, ??, TRUE? (really not sure about any of these answers!)
