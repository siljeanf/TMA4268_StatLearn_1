...
svm.linear = svm(formula = d.train$diabetes ~ ., 
                 data = d.train, 
                 kernel = 'linear')

#fit a support vector machine (radial boundary)
svm.radial = svm(formula = d.train$diabetes ~ ., 
                 data = d.train, 
                 kernel = 'radial')

#CV to find good cost parameter and gamma
cv.linear = tune(svm.linear, d.train$diabetes ~ ., data=d.train, kernel = "linear", ranges = list(cost = c(0.001,0.01,0.1,1,5,10,100 )))

tune.out <- tune(svm.radial, d.train$diabetes ~., data = d.train, kernel = "radial",
                 ranges = list(cost = c(0.1,1,10,100,1000), gamma = c(0.5,1,2,3,4) ))

#CV to find good cost and
