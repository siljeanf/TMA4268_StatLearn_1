...
$$
\begin{aligned}
&b_0(X) = 1 && b_4(X, q_1) = (X-q_1)_{+}^3 \\
&b_1(X) = X && b_5(X, q_2) = (X-q_2)_{+}^3 \\
&b_2(X) = X^2 && b_6(X, q_3) = (X-q_3)_{+}^3 \\
&b_3(X)= X^3
\end{aligned}
$$
...
b(X, q_j) = (x_i-q_j)_{+}^3
  ...
  \end{cases}
$$
This amounts to estimating $7$ regression coefficients $\beta_k$ for $k=0,..,7$ to the intercept and predictors.
...
In order to choose the optimal number of degrees of freedom , which is$df = `r df`$, we did cross validation.
