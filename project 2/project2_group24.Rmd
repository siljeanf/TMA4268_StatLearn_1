...
MSE_train_poly
MSE_poly_train_best = MSE_train_poly[9]
#training MSE for smoothing spline
MSE_spline_train

```

If we look back at the plots from c) we can see that when we plotted $Outstate$  against $Expend$ we had way less noise than with $Terminal$. Therefore we expected a smaller MSE for the model that uses $Expend$. Also, from the plot in d) (i) of the polynomial regression,
it seems like all degrees of polynomials were somehow similar, at least we cannot see a huge difference from the plot. So even though we did expect the MSE to decrease with higher degrees, we thought it would not decrease too much, which seems to be the case. Finally the plot from d) (ii) indicated a pretty good fit.
...
## e)
We now will use the results from PCA to find which genes vary the most accross the two groups. For this, we refer to the score vectors obtained by PCA. The genes that vaary the most accross the two groups are the ones that have the highest scores. We will display the 5 genes that vary most for the first principal components and the same for the second principal components.
<!-- Is this really true?? Is it ok to pick 5 for each? Which number should we choose? More for 1st or 2nd components? -->
