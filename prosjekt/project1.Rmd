...
ggplot(r2.tennis,aes(x=ACEdiff,y=UFEdiff ,color=factor(Result))) + geom_point() +
  theme_bw() + geom_abline(slope=b, intercept=a)+ ggtitle("Glm decision boundary")+theme(plot.title = element_text(hjust = 0.5))
```
...
```
A confidence interval is calculated given a set of observations in order to consider the true value for chd and if it is laying in the interval.This means that the true probability of chd for a male with sbp=140 would be found in this interval with probability 0.95. We notice that the interval is a bit wide, this means that the there is a great uncertantity when predicting the chd for a male with sbp=140.
