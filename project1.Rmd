...
t
```
The confusion table shows the performance of the classification rule, logisitc regression. The sum of the diagonal is the total number of correct classifications. The sum of all elements off the diagonal is the total number of misclassifications. We notice that there are some misclassifications for both classes. We will now calculate the sensitivity and specificity in order to understand this table better.
...
\end{aligned}
$$
The calculations between the second and third last line removes some terms. Here we use that $\Sigma$ is a positive semidefinitt matrix: $\Sigma^{-1} = (\Sigma^{-1})^T = (\Sigma^T)^{-1} = \Sigma^{-1}$. It then follows that two of the terms are equal, $\Big(\mu_0^T\Sigma^{-1}x\Big)^T = x^T\Sigma^{-1}\mu_0$. 
...
We will now make a plot with both the training and test observations with class boundary for LDA.
The data from the test set is represented with triangles, while the training data are circles in the plot.

```{r}
ggplot(r2.tennis,aes(x=ACEdiff,y=UFEdiff ,color=Result)) + geom_point(data = tennisTrain, shape="circle") + geom_point(data=tennisTest,shape="triangle")+ theme_bw() + geom_abline(slope=a/c, intercept=b/c)
...
table2
```
The confusion tables shows that most of the predictions are correct, with a misclassification of 9 and 5, there are a remarkeble chance for the prediction to be wrong.
...
## h)
We will now compare the plots for the different classification approaches we have discussed: logistic regression, LDA and QDA.

We know that QDA assumes a quadratic decision boundary. Usually this classfication rule is recommended when we have a huge amount of data for the training set so that the variance of the classifier is not a major concern, or if the assumption of common covariance matric is wrong. In this case it looks like the quadratic line overfits the data points. 

We now look at the linear decisions boundaries form LDA and logistic regression. We notice that the linear boundaries are at least as good as the quadratic decision boundary. Remember that LDA assumes that the observations are drawn from a Gaussian distribution with common covariance matrix in each class. It can seem like this assumption is not perfectly accurate as the boundary line produced from logistic regression fits the data points best.

The confusion matrices confirms the choice for glm as we can see that there are less misclassifications with logistic regression as classifier. 
...
print(c("The estimated probability of chd for a male with a sbp=140 in the given dataset is", p ))
```
...
```
A confidence interval is calculated given a set of observations in order to say something about the true value for chd laying in the interval.This means that the true probability of chd for a male with sbp=140 would be found in this interval with probability 0.95.. In this case the interval is quite big, this means that the there is a great uncertantity when predicting the chd. 
